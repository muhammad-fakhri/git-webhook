...
app.get("/", (req, res) => {
	res.json({ message: "Welcome to Git Webhook by Muhammad Fakhri" });
});
...
app.post("/webhook", (req, res) => {
	...
	if (!payload.username ||
		...
		!payload.workdir ||
		!payload.secret) {
		return res.status(400).json({ error: true, message: "Payload data is not meet requirement" });
	}
...
app.listen(port, () => console.log(`Git webhook server running at port ${port}`));